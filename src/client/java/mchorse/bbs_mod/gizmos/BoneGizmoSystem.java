...
public class BoneGizmoSystem
{
    ...
    /* Escala configurable del gizmo (tamaño base multiplicado por las medidas internas) */
    ...
    public void update(UIContext input, Area viewport, UIPropTransform target)
    {
        ...
        this.gizmoScale = Math.max(0.1F, BBSSettings.modelBlockGizmoScale.get());
        ...
        if (this.dragging && this.target != null && this.target.getTransform() != null)
        {
            ...
            {
                case TRANSLATE -> 0.03F;
                case SCALE -> 0.01F;
                case ROTATE -> 0.3F;
                case PIVOT -> 0.025F;
                case UNIVERSAL -> {
                    Mode opInner = (this.activeSubMode != null) ? this.activeSubMode : Mode.TRANSLATE;
                    yield (opInner == Mode.TRANSLATE) ? 0.03F : (opInner == Mode.SCALE ? 0.01F : 0.3F);
                }
            };
    ...
    public void render3D(MatrixStack stack)
    {
        ...
        float baseLength = 0.16F;
        float length = baseLength * this.gizmoScale;     // longitud de cada eje
        float thickness = 0.012F * this.gizmoScale;      // barras más delgadas
        float slabThick = 0.014F * this.gizmoScale;      // losas más delgadas
        ...
        float cubeBig = 0.038F * this.gizmoScale;
        ...
        if (this.mode == Mode.SCALE)
        {
            ...
            if (showX) Draw.fillBoxTo(builder, stack, 0, 0, 0, length + connectFudge, 0, 0, thickness, 1F, 0F, 0F, 1F);
            if (showY) Draw.fillBoxTo(builder, stack, 0, 0, 0, 0, length + connectFudge, 0, thickness, 0F, 1F, 0F, 1F);
            if (showZ) Draw.fillBox(builder, stack, -thickness / 2F, -thickness / 2F, 0F, thickness / 2F, thickness / 2F, length + connectFudge, 0F, 0F, 1F, 1F);
        }
        ...
        if (this.mode == Mode.TRANSLATE || this.mode == Mode.PIVOT)
        {
            ...
            float headLen = 0.05F * this.gizmoScale;       // altura del cono (más delgado)
            ...
            float sphereR = 0.036F * this.gizmoScale;
            ...
            if (showX) Draw.fillBoxTo(builder, stack, 0, 0, 0, barEnd, 0, 0, txX, 1F, 0F, 0F, 1F);
            if (showY) Draw.fillBoxTo(builder, stack, 0, 0, 0, 0, barEnd, 0, txY, 0F, 1F, 0F, 1F);
            if (showZ) Draw.fillBox(builder, stack, -txZ / 2F, -txZ / 2F, 0F, txZ / 2F, txZ / 2F, barEnd, 0F, 0F, 1F, 1F);

            // Manejadores en la punta: flechas para TRANSLATE, esferas para PIVOT
            if (this.mode == Mode.TRANSLATE)
            {
                if (showX) drawCone3D(builder, stack, 'X', lengthBar, headLen, headRadius, 1F, 0F, 0F, 1F);
                if (showY) drawCone3D(builder, stack, 'Y', lengthBar, headLen, headRadius, 0F, 1F, 0F, 1F);
                if (showZ) drawCone3D(builder, stack, 'Z', lengthBar, headLen, headRadius, 0F, 0F, 1F, 1F);
            }
            else // PIVOT: usar esferas para evitar confusión con TRASLADAR
            {
                if (showX) drawSphere3D(builder, stack, 'X', lengthBar, sphereR, 1F, 0F, 0F, 1F);
                if (showY) drawSphere3D(builder, stack, 'Y', lengthBar, sphereR, 0F, 1F, 0F, 1F);
                if (showZ) drawSphere3D(builder, stack, 'Z', lengthBar, sphereR, 0F, 0F, 1F, 1F);
            }
        ...
        {
            Mode usingSub = this.dragging ? this.activeSubMode : this.hoveredSubMode;
            ...
            boolean showTranslate = (usingSub == null || usingSub == Mode.TRANSLATE);
            boolean showRotate = (usingSub == null || usingSub == Mode.ROTATE);

            float barEnd = length + connectFudge - (headLen * 0.25F);
            float lengthBar = barEnd + (this.mode == Mode.TRANSLATE ? headLen : 0F);

            if (showTranslate)
            {
                if (showX) { Draw.fillBoxTo(builder, stack, 0, 0, 0, barEnd, 0, 0, hx ? thickness * 1.5F : thickness, 1F, 0F, 0F, 1F); drawCone3D(builder, stack, 'X', lengthBar, headLen, headRadius, 1F, 0F, 0F, 1F); }
                if (showY) { Draw.fillBoxTo(builder, stack, 0, 0, 0, 0, barEnd, 0, hy ? thickness * 1.5F : thickness, 0F, 1F, 0F, 1F); drawCone3D(builder, stack, 'Y', lengthBar, headLen, headRadius, 0F, 1F, 0F, 1F); }
                if (showZ) { Draw.fillBox(builder, stack, -(hz ? thickness * 1.5F : thickness) / 2F, -(hz ? thickness * 1.5F : thickness) / 2F, 0F, (hz ? thickness * 1.5F : thickness) / 2F, (hz ? thickness * 1.5F : thickness) / 2F, barEnd, 0F, 0F, 1F, 1F); drawCone3D(builder, stack, 'Z', lengthBar, headLen, headRadius, 0F, 0F, 1F, 1F); }
            }
            ...
            if (showRotate)
            {
                ...
                float thicknessRing = 0.007F * this.gizmoScale;
                float sweep = 360F;
        ...
        else if (this.mode == Mode.ROTATE)
        {
            ...
            float offX = 0F;
            float offY = 0F;
            float ringThickness = 0.007F * this.gizmoScale;
    ...
    private Axis detectHoveredAxis3D(UIContext input, Area viewport, Matrix4f origin, Matrix4f projection, Matrix4f view)
    {
        ...
        if (this.mode == Mode.UNIVERSAL)
        {
            ...
            // Detectar traslación sobre barras dentro del anillo
            float len = 0.16F * this.gizmoScale;
            float thickness = 0.014F * this.gizmoScale;
            float fudge = 0.08F * this.gizmoScale;

            float[] tx = rayBoxIntersect(rayO, rayD, new Vector3f(0F, -thickness / 2F, -thickness / 2F), new Vector3f(len + fudge, thickness / 2F, thickness / 2F));
            float[] ty = rayBoxIntersect(rayO, rayD, new Vector3f(-thickness / 2F, 0F, -thickness / 2F), new Vector3f(thickness / 2F, len + fudge, thickness / 2F));
            float[] tz = rayBoxIntersect(rayO, rayD, new Vector3f(-thickness / 2F, -thickness / 2F, 0F), new Vector3f(thickness / 2F, thickness / 2F, len + fudge));

            float bt = Float.POSITIVE_INFINITY; Axis ba = null;
            if (tx != null && tx[0] >= 0 && tx[0] < bt) { bt = tx[0]; ba = Axis.X; }
            if (ty != null && ty[0] >= 0 && ty[0] < bt) { bt = ty[0]; ba = Axis.Y; }
            if (tz != null && tz[0] >= 0 && tz[0] < bt) { bt = tz[0]; ba = Axis.Z; }
            if (ba != null) { this.hoveredSubMode = Mode.TRANSLATE; this.hoveredPlane = null; return ba; }

            this.hoveredSubMode = null; this.hoveredPlane = null; return null;
        }

        // Definir AABB por eje (mover/escalar)
        float length = 0.16F * this.gizmoScale;
        // En escala el cubo del extremo es más grande; ampliamos sección transversal del AABB
        float thickness = ((this.mode == Mode.SCALE) ? 0.038F : 0.014F) * this.gizmoScale;
        float fudge = (((this.mode == Mode.TRANSLATE) || (this.mode == Mode.PIVOT)) ? 0.08F : ((this.mode == Mode.SCALE) ? 0.045F : 0.02F)) * this.gizmoScale;
    ...
    private Axis detectHoveredAxis3DRotate(Vector3f rayO, Vector3f rayD)
    {
        ...
        float baseThickness = ((this.mode == Mode.ROTATE) ? 0.007F : 0.006F) * this.gizmoScale;
    ...
    public void renderOverlay(UIRenderingContext context, Area viewport)
    {
        ...
        else if (this.mode == Mode.UNIVERSAL)
        {
            ...
            Mode usingSub = this.dragging ? (this.activeSubMode != null ? this.activeSubMode : Mode.TRANSLATE) : this.hoveredSubMode;
    ...
    private Axis detectHoveredAxis(int mx, int my)
    {
        ...
        if (this.mode == Mode.UNIVERSAL)
        {
            ...
            if (isNearLine(mx, my, cx, cy, endXx, endXy, tol) || isNear(mx, my, endXx, endXy, hitRadius)) { this.hoveredSubMode = Mode.TRANSLATE; this.hoveredPlane = null; return Axis.X; }
            if (isNearLine(mx, my, cx, cy, endYx, endYy, tol) || isNear(mx, my, endYx, endYy, hitRadius)) { this.hoveredSubMode = Mode.TRANSLATE; this.hoveredPlane = null; return Axis.Y; }
            if (isNearLine(mx, my, cx, cy, endZx, endZy, tol) || isNear(mx, my, endZx, endZy, hitRadius)) { this.hoveredSubMode = Mode.TRANSLATE; this.hoveredPlane = null; return Axis.Z; }
